...
- Half-precision GEMM: `blas::hgemm` (issue #47), the fp16 counterpart of
  ...
  `&[u16]` holding IEEE-754 binary16 bit patterns, because that is what oneMKL's
  element type is: `mkl_cblas.h` declares `cblas_hgemm`'s operands and both
  scalars as `MKL_F16`, and `mkl_types.h` defines that as `unsigned short` —
  not a native half float (Rust's `f16` is still unstable, and is a distinct
  type at an FFI boundary regardless). **Verified present in oneMKL 2026.1.0**,
  which was the open question in the issue: the symbol is declared at
  `mkl_cblas.h:980` and defined in the shipped libraries, along with the Fortran
  `hgemm_` and the mixed-precision `cblas_gemm_bf16bf16f32`/`gemm_f16f16f32`
  entry points this wrapper does not yet expose. It is a oneMKL extension to
  CBLAS rather than netlib, so neither ARM64 backend has it: `hgemm` returns
  `ErrorKind::Unsupported` on `aarch64-apple-darwin` (Accelerate's CBLAS has no
  fp16 routine) and on `aarch64-unknown-linux-gnu` (OpenBLAS 0.3.30 exposes no
  `hgemm`/`cblas_hgemm` symbol) — before validating its arguments, so feature
  detection does not depend on the operands.

- Binary element-wise VML operations: `add`/`dadd`, `sub`/`dsub`, `mul`/`dmul`,
  `div`/`ddiv`, `fmax`/`dfmax` and `fmin`/`dfmin` (issue #48), generated by a
  new `vml_binary!` macro that mirrors `vml_unary!`. Each takes two equal-length
  operands and a distinct output slice, and validates all three lengths through
  the same `i32::MAX` guard the unary path uses. On Intel they call oneMKL's
  `vsAdd`/`vdAdd`, `vsSub`/`vdSub`, `vsMul`/`vdMul`, `vsDiv`/`vdDiv`,
  `vsFmax`/`vdFmax` and `vsFmin`/`vdFmin`; on Apple Silicon they call vDSP,
  **not** vForce — a survey of the SDK's 84 `vv*` symbols found vForce carries
  only `vvdiv` of the six, so mapping all six to vDSP's uniform
  `(A, IA, B, IB, C, IC, N)` shape is what keeps the surface complete. Two of
  those (`vDSP_vsub`, `vDSP_vdiv`) are declared with their inputs *swapped*
  relative to their names; the wrapper compensates, and `vml_binary_ops_f32`/
  `_f64` use asymmetric operands so a dropped swap would fail rather than pass
  quietly. `ErrorKind::Unsupported` on `aarch64-unknown-linux-gnu`.

- Unary VML additions: `tanh`/`dtanh` and `sqr`/`dsqr` (issue #49). `tanh` maps
  to `vsTanh`/`vdTanh` on Intel and vForce's `vvtanhf`/`vvtanh` on Apple
  Silicon — confirmed present in the SDK, as the issue asked — while `sqr` maps
  to `vsSqr`/`vdSqr` on Intel and **vDSP's `vDSP_vsq`/`vDSP_vsqD`** on Apple
  Silicon, because vForce has no squaring function at all. Both oneMKL symbols
  were verified in the 2026.1.0 headers before wiring, and each function's
  per-backend mapping is recorded in its docs and in the README's
  domain-coverage table.

- `vml::fmax`/`fmin` and their double-precision forms carry a documented
  cross-backend difference rather than a silent one: oneMKL's `vsFmax`/`vsFmin`
  return the non-`NaN` operand when exactly one of a pair is `NaN`, while
  Accelerate's `vDSP_vmax`/`vDSP_vmin` propagate it (measured against the
  framework, and recorded in the README alongside the VSL generator difference
  ADR-0003 already documents).
