...
//!
//! [`Stream`] wraps a VSL random stream. Streams hold raw state and are
//! therefore neither `Send` nor `Sync`; share one behind a lock when
//! cross-thread randomness is required.
...
/// A VSL random-number stream.
pub struct Stream {
    state: nuvai_mkl_sys::VSLStreamStatePtr,
}
